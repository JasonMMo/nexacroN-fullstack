...
public class CharsetUtil {
    ...
    private static final Properties languageMappingProperties = new Properties();
    private static final Logger logger = LoggerFactory.getLogger(CharsetUtil.class);

    static {

        String languageMappingResourceName = "language_mapping.properties";
        try (InputStream inputStream = CharsetUtil.class.getResourceAsStream(languageMappingResourceName)) {
    ...
    private static String getCharsetOfAcceptCharset(String acceptCharsets) {
        ...
        for (int i = 0; i < charsetValues.length; i++) {
            ...
            if (index >= 0) {
                ...
                quality = Double.parseDouble(charsetValues[i].substring(index + ";q=".length()));
            }
    ...
    private static String getCharsetOfAcceptLanguage(String languagevalue) {
        ...
        for (int i = 0; i < languageValues.length; i++) {
            ...
            if (index >= 0) {
                ...
                quality = Double.parseDouble(languageValues[i].substring(index + ";q=".length()));
            }

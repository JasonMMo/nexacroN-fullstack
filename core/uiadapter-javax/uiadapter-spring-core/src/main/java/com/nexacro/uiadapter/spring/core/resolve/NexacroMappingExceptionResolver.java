...
public class NexacroMappingExceptionResolver extends AbstractHandlerExceptionResolver {
     ...
     * 
     * @param defaultErrorMsg 예외시 기본 에러메시지
     ...
     * 
     * @param shouldSendStackTrace  해당 메시지가 전송 유무
	 ...
	 * @param shouldLogStackTrace 예외 정보를 로깅할지 유무
	 */
	...
	protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
	    ...
	    if(NexacroUtil.isNexacroRequest(request)
	    		// 2022.07.11 FileDownload시 발생하는 Exception 처리위해 추가.
	    		// 				에러는 반드시 NexacroException으로 던져야 한다.
	    		|| ex instanceof NexacroException) {

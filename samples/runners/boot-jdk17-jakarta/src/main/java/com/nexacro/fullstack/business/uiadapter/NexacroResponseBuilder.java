...
public final class NexacroResponseBuilder {
    ...
    public static NexacroEnvelope ok(NexacroDataset... datasets) {
        List<NexacroDataset> list = (datasets == null || datasets.length == 0)
                ? new ArrayList<>()
                : new ArrayList<>(Arrays.asList(datasets));
        return buildOk(list);
    }
    ...
    public static NexacroEnvelope ok(List<NexacroDataset> datasets) {
        List<NexacroDataset> list = (datasets != null) ? datasets : new ArrayList<>();
        return buildOk(list);
    }
    ...
    public static NexacroEnvelope error(int code, String message) {
        List<NexacroEnvelope.Parameter> params = new ArrayList<>();
        params.add(new NexacroEnvelope.Parameter("ErrorCode", code, "int"));
        params.add(new NexacroEnvelope.Parameter("ErrorMsg",
                message != null ? message : "", "string"));
        return new NexacroEnvelope("1.0", params, new ArrayList<>());
    }
    ...
    private static NexacroEnvelope buildOk(List<NexacroDataset> datasets) {
        List<NexacroEnvelope.Parameter> params = new ArrayList<>();
        params.add(new NexacroEnvelope.Parameter("ErrorCode", 0, "int"));
        params.add(new NexacroEnvelope.Parameter("ErrorMsg", "", "string"));

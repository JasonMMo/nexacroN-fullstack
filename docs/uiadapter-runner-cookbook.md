...
| 7 | 기동 시 `IndexOutOfBoundsException: Index -1 out of bounds for length 27` (PR #24, **javax lane only**) | PR #22 가 split context-*.xml + dispatcher-servlet.xml 을 추가했지만 web.xml 이 여전히 stale `AnnotationConfigWebApplicationContext + RootConfig/WebMvcConfig` 를 가리키고 Java `@Configuration` 6개 (`DataSourceConfig`, `MyBatisConfig`, `RelayHttpConfig`, `RootConfig`, `UiadapterWebMvcConfig`, `WebMvcConfig`) 가 classpath 에 남아 있음 → `WebMvcConfig.requestMappingHandlerAdapter()` 가 XML `NexacroRequestMappingHandlerAdapter` 빈과 동시에 등록되며 충돌 | web.xml 을 XmlWebApplicationContext (default) 로 전환 + stale Java config 6개 삭제. `CharacterEncodingFilter` `*.do` 추가 (jakarta lane parity), `<servlet>` XExportImport 제거 (dispatcher 의 `ServletWrappingController` 가 처리). 검증: POST `/uiadapter/select_datalist.do` (Content-Type: text/xml, 빈 PlatformXml 봉투) → 200 + nexacro envelope + ErrorCode=0 + populated output1 |

**XML 변환 체크리스트** (lane 마다 PR 직전 필수):
- [ ] web.xml `<context-param>contextConfigLocation</context-param>` 가 split context-*.xml 7개를 모두 가리키는지
- [ ] web.xml 에 `contextClass` override **없음** (default XmlWebApplicationContext 사용)
- [ ] Java `@Configuration` 클래스가 classpath 에 **잔존 없음** (component-scan 으로 다시 잡혀 XML 빈과 충돌)
- [ ] dispatcher servlet 의 `<init-param>` `contextConfigLocation` 이 `/WEB-INF/config/springmvc/dispatcher-servlet.xml` 1개만 가리키는지 (root 컨텍스트 파일을 또 로드하면 빈 중복 등록)
- [ ] CharacterEncodingFilter `*.do` 매핑 (jakarta + javax lane 둘 다, utf-8 강제)
...
### 9-MVC.4a 라이선스 드롭인 (실 envelope 응답 검증용)

xapi 가 `WEB-INF/classes` 에서 `NexacroN_server_license.xml` 을 classpath 룩업하므로, 라이선스 없이는 endpoint 가 항상 `InvalidLicenseException: License not found` → 500 반환. 실 envelope (HTTP 200 + ErrorCode=0 + populated output1) 을 보려면 다음 절차:

```powershell
# 1) 라이선스 파일을 WAR 내부 WEB-INF/classes 에 드롭 (Tomcat 의 expanded 디렉터리도 동일)
Copy-Item 'C:\path\to\NexacroN_server_license.xml' `
          "$env:CATALINA_BASE\webapps\uiadapter\WEB-INF\classes\NexacroN_server_license.xml"

# 2) Tomcat 재기동 (classpath cache 갱신)
& "$env:CATALINA_HOME\bin\catalina.bat" stop
& "$env:CATALINA_HOME\bin\catalina.bat" start

# 3) 빈 PlatformXml 봉투 smoke (Content-Type: text/xml 필수 — 이게 없으면 PlatformBinary 로 오인되어 EOFException)
@'
<?xml version="1.0" encoding="UTF-8"?>
<Root xmlns="http://www.nexacroplatform.com/platform/dataset"/>
'@ | Out-File -Encoding utf8 D:\AI\testspace\platform-xml-empty.xml
...
# -> 200 OK + <Parameters><Parameter id='ErrorCode'>0</Parameter>... + <Dataset id='output1'> 행 데이터
```

**공용 repo 안전성**: 모노레포 루트 `.gitignore` 가 `NexacroN_server_license.xml` rule 로 이미 보호 중이므로 실수로 commit 될 위험 없음. 그래도 라이선스 파일은 `samples/runners/*/src/main/resources/` 에는 **두지 말 것** — testspace 의 expanded `WEB-INF/classes/` 에만 떨어뜨려서 빌드 산출물에 포함되지 않게.
...
| #24 | fix(mvc-jdk8-javax): finish XML config conversion (web.xml + drop stale Java configs) | web.xml XmlWebApplicationContext 전환 + Java `@Configuration` 6개 제거 — 위 9-MVC.4 #7 |
| (this PR) | refactor(mvc): align xeniWrappingController + messageSource with GitLab canonical jakarta-example | xeniWrappingController `initParameters` 제거 (web.xml `<context-param>` 단일 출처) + `messageSource` (`ReloadableResourceBundleMessageSource`) + `message-common[_ko].properties` 추가 + `NexacroMappingExceptionResolver` 에 명시 wiring |

### 9-MVC.7 GitLab canonical jakarta-example 와의 비교

레퍼런스: `https://gitlab.com/nexacron/spring-framework/jakarta/nexacro-jakarta-example`

**구조적 parity** (= 의도적으로 동일하게 유지):

| 영역 | canonical jakarta-example | 본 런너 (mvc-jdk17-jakarta / mvc-jdk8-javax) |
|---|---|---|
| web.xml | ContextLoaderListener + DispatcherServlet + CharacterEncodingFilter `*.do` | 동일 |
| 컨텍스트 분리 | root: context-*.xml 다중 / dispatcher: dispatcher-servlet.xml 1개 | 동일 |
| Nexacro 빈 셋 | `NexacroInterceptor`, `NexacroFileView`, `NexacroView` (`PlatformXml` + UTF-8), `NexacroRequestMappingHandlerAdapter`, `NexacroMethodArgumentResolver`, `NexacroHandlerMethodReturnValueHandler`, `NexacroMappingExceptionResolver` (`fail.common.msg`) | 동일 |
| XENI 브리지 | `ServletWrappingController` + `SimpleUrlHandlerMapping` `/XExportImport.do` + servletClass-only (init-params from web.xml `<context-param>`) | 동일 (이 PR 에서 정렬) |
| i18n | `ReloadableResourceBundleMessageSource` (basename `classpath:/message/message-common`, UTF-8) + `NexacroMappingExceptionResolver.messageSource` 명시 wiring | 동일 (이 PR 에서 정렬) |
| controller component-scan | dispatcher 컨텍스트만 `@Controller` include, root 는 exclude | 동일 |

**의도적 deviation** (= GitLab canonical 과 다르게 유지, 본 모노레포 컨텍스트에 맞춘 결정):

| # | deviation | rationale |
|---|---|---|
| 1 | `<context:property-placeholder>` 사용 (canonical 은 `PropertyPlaceholderConfigurer`) | Spring 5+ 표준, 동일 효과, 더 짧음 |
| 2 | `ResourceDatabasePopulator` 의 `separator="^^"` 추가 | 모노레포 schema.sql/data.sql 의 statement separator. boot 런너 `spring.sql.init.separator` 와 parity |
| 3 | jakarta lane 에 `nexacroStreamView` 추가 (`NexacroStreamView`) | uiadapter-jakarta-core 1.0.27.1 가 노출하는 stream view (canonical 시점에는 없었음). javax lane 은 stream view 미노출이라 추가 안 함 |
| 4 | `context-relay.xml` (RelayController용) | 모노레포 nxui 의 relay 화면 endpoint 지원. canonical 은 relay 없음 |
| 5 | log4jdbc 미사용 | canonical 은 query 로깅용으로 활성. 모노레포는 mybatis 의 `<settings>` 로 충분, 의존성 한 줄 줄임 |
| 6 | JSP error page 없음 (`<error-page>` 누락) | canonical 은 `/WEB-INF/jsp/error.jsp` 보냄. nexacro 클라이언트는 envelope ErrorCode 만 보므로 HTML 페이지 불필요 |
| 7 | `localeResolver`, mvc validator, `urlBasedViewResolver` 등 부가 빈 미선언 | 본 런너의 모든 endpoint 가 `NexacroView` 로만 응답하므로 불필요 |

이전 PR 까지의 deviation 중 **이 PR 에서 해소된** 항목:
- ~~xeniWrappingController 가 `initParameters` 인라인 5개 + `servletName` 보유~~ → web.xml `<context-param>` 단일 출처로 정렬 (canonical 패턴 그대로)
- ~~messageSource 빈 없음 + `NexacroMappingExceptionResolver` 가 fallback 메시지로 동작~~ → `ReloadableResourceBundleMessageSource` 추가 + `fail.common.msg` 키가 ko/default 양쪽 bundle 로 resolve
...
_Last updated: 2026-05-12 (mvc-jdk17-jakarta / mvc-jdk8-javax XML config WAR + GitLab canonical jakarta-example 정렬 완료 — xeniWrappingController, messageSource, i18n bundle. Tomcat 10/9 parity, PR #23 + #24 + xeni/messageSource alignment PR)_

...
| 7 | 기동 시 `BeanCreationException: requestMappingHandlerAdapter ... IndexOutOfBoundsException: Index -1 out of bounds for length 27` (**javax lane PR #22 회귀**) | XML 변환 시 web.xml 은 `AnnotationConfigWebApplicationContext` + `RootConfig` / `WebMvcConfig` 로 남고, 새로 추가한 `dispatcher-servlet.xml` 의 `<bean class="...NexacroRequestMappingHandlerAdapter" p:order="0"/>` 와 Java `@Bean` 이 동시에 등록되어 어댑터 초기화 충돌 | (a) web.xml 의 `contextClass` 제거 (default `XmlWebApplicationContext`), (b) `contextConfigLocation` 을 `classpath:spring/context-*.xml` 7 개 + `/WEB-INF/config/springmvc/dispatcher-servlet.xml` 로 변경, (c) `config/{DataSource,MyBatis,RelayHttp,Root,UiadapterWebMvc,WebMvc}Config.java` 6 개 **모두 삭제** (RelayProperties.java 는 @Component POJO 라 유지). PR #24 (`hotfix/mvc-jdk8-javax-xml-config`) 참조. |
...
**XML 변환 체크리스트 (한 lane 을 보내기 전 마지막 확인)**:
1. `web.xml` — `contextClass` 속성이 없어야 함 (default `XmlWebApplicationContext`)
2. `web.xml` — root `contextConfigLocation` 이 `classpath:spring/context-*.xml` 만 가리킴
3. `web.xml` — dispatcher `contextConfigLocation` 이 `/WEB-INF/.../dispatcher-servlet.xml` 만 가리킴
4. `src/main/java/.../config/` — `@Configuration` annotated 파일이 **0 개** (POJO/@Component 만 남음)
5. `mvn package` 후 `unzip -l target/uiadapter.war | grep -E 'config/(DataSource|MyBatis|Relay|Root|Uiadapter|Web)\b'` 결과가 비어 있어야 함

### 9-MVC.4a 라이선스 드롭인 (실 envelope 응답을 보려면 필수)

XML wiring 이 정확하더라도 `xapi.license` 가 라이선스를 찾지 못하면 `InvalidLicenseException` 으로 500 응답을 낸다 (boot 런너와 동일 동작 — wiring parity 판정에는 이게 정상 시그널이다). 정상 nexacro envelope 응답을 확인하려면 운영자가 발급한 라이선스를 다음 경로에 드롭한다:

```
samples/runners/<KEY>/src/main/resources/NexacroN_server_license.xml
```

빌드 후 WAR 의 `WEB-INF/classes/NexacroN_server_license.xml` 로 들어가고, `com.nexacro.java.xapi.license.A.E` 의 search path (1: JAR 위치, 2: classpath) 중 두 번째에서 hit. 이 위치는 `.gitignore` 의 `NexacroN_server_license.xml` 룰로 이미 보호되고 있으므로 실수로 public repo 에 push 될 위험 없음. 라이선스가 잡힌 후의 smoke 요청 예 (jakarta/javax 동일):

```powershell
# 빈 PlatformXml 봉투 — POST body 가 form-urlencoded 면 "PlatformBinary" 로 잘못 잡혀 0x6d65 deserialize 오류 발생
$xml = @'
<?xml version="1.0" encoding="UTF-8"?>
<Root xmlns="http://www.nexacroplatform.com/platform/dataset">
  <Parameters/>
  <Dataset id="ds_search"><ColumnInfo/><Rows/></Dataset>
</Root>
'@
$xml | Out-File -Encoding utf8 D:\AI\testspace\platform-xml-empty.xml

curl.exe -sv -X POST http://localhost:8080/uiadapter/select_datalist.do `
  -H "Content-Type: text/xml" `
  --data-binary "@D:\AI\testspace\platform-xml-empty.xml"
# 기대: HTTP 200 + <Parameter id="ErrorCode" type="int">0</Parameter> + <Dataset id="output1"> 에 board 행 채워짐
```
...
| #24 | fix(mvc-jdk8-javax): switch web.xml to XmlWebApplicationContext + drop stale `@Configuration` 6 files | PR #22 가 dispatcher-servlet.xml + context-*.xml 7 개를 추가하면서 web.xml 과 Java config 6 개를 정리하지 못해 Spring bean 충돌로 `requestMappingHandlerAdapter` 초기화에서 IndexOutOfBoundsException — 위 9-MVC.4 #7 |

### 9-MVC.7 GitLab 캐노니컬 jakarta-example 과의 비교

레퍼런스: <https://gitlab.com/nexacron/spring-framework/jakarta/nexacro-jakarta-example>

본 모노레포의 `mvc-jdk17-jakarta` 및 `mvc-jdk8-javax` 런너는 위 GitLab 캐노니컬 예제의 XML wiring 구조를 그대로 따른다. 구조적 동일성:

- `web.xml` — 기본 `XmlWebApplicationContext` (contextClass override 없음) + `ContextLoaderListener` + `DispatcherServlet` (`*.do`) + multipart-config + `CharacterEncodingFilter(utf-8)`
- root context = `classpath:spring/context-*.xml` 분할 로드 / dispatcher context = `WEB-INF/config/springmvc/dispatcher-servlet.xml`
- Nexacro bean set: `applicationContextProvider`, 5× `Dbms` (Hsql/Oracle/Mssql/Mysql/Tibero) + `DbVendorsProvider`, `NexacroInterceptor`, `NexacroFileView`, `NexacroView`, `NexacroRequestMappingHandlerAdapter` (`p:order=0`) + `NexacroMethodArgumentResolver` + `NexacroHandlerMethodReturnValueHandler`, `NexacroMappingExceptionResolver` (`p:order=1`)
- root component-scan: `@Controller` 제외 / dispatcher component-scan: `@Controller` 만 (parent-child 컨텍스트 분리)
- transaction AOP: `<tx:advice>` + `<aop:config>` pointcut `*Impl.*(..)`
- XENI: `GridExportImportServlet` 을 `ServletWrappingController` + `SimpleUrlHandlerMapping(/XExportImport.do)` 로 래핑

의도된 차이점 (런너 요건상 캐노니컬에서 벗어남):

| 영역 | 캐노니컬 | 본 런너 | 사유 |
|---|---|---|---|
| 설정 외부화 | DataSource/Mapper 값 하드코딩 | `<context:property-placeholder location="classpath:application.properties"/>` | 런너 빌드 시 환경별 override 용이 |
| HSQLDB SQL 분할 | 5 개 파일 (`create_tb_*.sql`, `insert_tb_*.sql`), separator 기본 `;` | 2 개 파일 (`schema.sql`, `data.sql`), `separator="^^"` | seed-data 단순화 + MULTI-STATEMENT 지원 |
| dispatcher view set | `view`, `fileView` 만 | + `streamView` (uiadapter `NexacroStreamView`) | 대용량 응답 / progress stream API 지원 (uiadapter 1.0.27.1+) |
| Relay (EXIM) 컨텍스트 | 없음 | `context-relay.xml` 추가 | 본 모노레포의 외부 시스템 relay 시나리오 |
| `xeniWrappingController` 파라미터 | `servletClass` 만 (web.xml `<context-param>` fallback) | `servletClass` + `initParameters{export-path,import-path,monitor-*,file-storage-time}` + `servletName` | web.xml `<context-param>` 누락 가능성 방어 (web.xml 와 dispatcher 양쪽 명시) |
| i18n 인프라 | `messageSource` + `localeResolver` + `validator` + `urlBasedView`(JSP) | 모두 미설치 | nexacro envelope REST 응답만 다루므로 JSP/i18n 미사용 |
| `defaultErrorMsg` | `messageSource` 키 `fail.common.msg` (메시지 번들 lookup) | 인라인 문자열 | messageSource 부재 |
| 로그 wrapper | `net.sf.log4jdbc.DriverSpy` JDBC 래핑 | 직접 `org.hsqldb.jdbc.JDBCDriver` | log4jdbc 의존성 미추가 (smoke/CI 용도) |
| 보안/에러 페이지 | `<login-config>BASIC` + 3× `<error-page>/common/error.jsp` | 없음 | 런너는 REST-only — JSP 에러 페이지 불필요 |

검증된 동작 (양쪽 lane, 2026-05-12):
- `POST /uiadapter/select_datalist.do` (Content-Type: `text/xml`, body = 빈 PlatformXml envelope) → HTTP 200 + `<Parameter id="ErrorCode" type="int">0</Parameter>` + populated `<Dataset id="output1">`
- jakarta = Tomcat 10 + JDK 17, javax = Tomcat 9 + JDK 17 (isolated CATALINA_BASE)
- 라이선스: `src/main/resources/NexacroN_server_license.xml` (`com.nexacro.java.xapi.license.A.E` 의 classpath lookup hit)

향후 캐노니컬과 더 가깝게 맞추려면 (선택):
1. `xeniWrappingController` 의 `initParameters`/`servletName` 제거 → web.xml `<context-param>` fallback 만 의존 (XENI Excel export smoke 후 반영)
2. `messageSource` + `message-common.properties` 추가 → `NexacroMappingExceptionResolver` 의 `defaultErrorMsg="fail.common.msg"` 로 전환
3. HSQLDB seed 분할 (5 SQL + `;` separator) — 단, `^^` 의 MULTI-STATEMENT 케이스가 없으면 의미 없음
...
_Last updated: 2026-05-12 (mvc-jdk17-jakarta + mvc-jdk8-javax 양쪽 XML config WAR 실 envelope 응답 검증 완료 — HTTP 200 + nexacro Dataset, Tomcat 10 / Tomcat 9 parity, PR #23 + PR #24)_
